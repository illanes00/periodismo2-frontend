      ...
      <nav className="sticky top-0 z-50 border-b border-neutral-200/80 bg-white/90 backdrop-blur-lg dark:border-neutral-800/80 dark:bg-neutral-950/90">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 md:gap-8">
            ...
            {/* Desktop nav: Portada | top 6 categories | Más ▾ | Editorial | Nosotros */}
            <div className="hidden items-center gap-1 md:flex">
              <Link href="/" className={linkClass}>
                Portada
              </Link>

              {/* Top 6 categories as direct links */}
              {NAV_CATEGORIES.slice(0, 6).map((cat) => (
                <Link key={cat.slug} href={`/category/${cat.slug}`} className={linkClass}>
                  {cat.label}
                </Link>
              ))}
              ...
              <div ref={dropdownRef} className="relative">
                ...
                {dropdownOpen && (
                  <div className="absolute left-0 top-full mt-1 w-48 rounded-xl border border-neutral-200/80 bg-white py-1 shadow-lg dark:border-neutral-800/80 dark:bg-neutral-900">
                    {NAV_CATEGORIES.slice(6).map((cat) => (
                      <Link
                        key={cat.slug}
                        href={`/category/${cat.slug}`}
                        className="block px-4 py-2 text-sm text-neutral-700 transition hover:bg-neutral-100 hover:text-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-neutral-100"
                      >
                        {cat.label}
                      </Link>
                    ))}
